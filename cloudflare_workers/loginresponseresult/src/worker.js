...
async function handleRequest(request) {
	if (request.method === 'POST' && request.url.endsWith('/api/v1/auth/login')) {
		...
		if (isComplete) {
			...
			const accessToken = await generateJwtToken({ sub: username });
			const refreshToken = await generateJwtToken({ sub: username, type: 'refresh' });
...
async function generateJwtToken(payload) {
	...
	const signature = base64urlEncode(await hmacSHA256(`${encodedHeader}.${encodedToken}`, secretKey));
